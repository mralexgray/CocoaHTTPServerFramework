...
@implementation JQueriedResponse { BOOL done; 	NSData *separator2;	NSDictionary *replacementDict; }

- (id)initWithFilePath:(NSString *)fileP  forConnection:(HTTPConnection *)conn {

	self = [super initWithFilePath:fileP forConnection:conn separator:@"<script>" replacementDictionary:@{@"JQUE": @""}];
	separator2 = [@"</script>" dataUsingEncoding:NSUTF8StringEncoding]; return self;
}

/*- (void)processReadBuffer {		HTTPLogTrace();

	// At this point, the readBuffer has readBufferOffset bytes available. This method is in charge of updating the readBufferOffset.
	NSUInteger bufLen = readBufferOffset, sepLen = separator.length;
	...
	NSUInteger offset = 0, stopOffset = (bufLen > sepLen) ? bufLen - sepLen + 1 : 0;
	...
	const void *sep = [separator bytes], *sep2 = [separator2 bytes];
	...
	while (offset < stopOffset)	{
		...
		if (memcmp(subBuffer, sep, sepLen) == 0)
		{
			...
			if (found1 && found2)
			{
				...
				// Wish we could use the simple subdataWithRange method. But that method copies the bytes...
...
{
	if (!(self = [super initWithFilePath:fpath forConnection:parent])) return nil;
	HTTPLogTrace(); 	separator = [separatorStr dataUsingEncoding:NSUTF8StringEncoding]; replacementDict = dict;	return self;
}

- (BOOL)isChunked	{	HTTPLogTrace();	return YES;	}

#define DUMMYRESPONSE(X) 	HTTPLogTrace(); return X
- (UInt64) contentLength						{ DUMMYRESPONSE(0);	} // This method shouldn't be called since we're using a chunked response.
-   (void) setOffset:(UInt64)offset	{	DUMMYRESPONSE();	} // We override it just to be safe.
